...
const HandleAddInventory = async (req, res) => {
    ...
    try {
        ...
        if(!productName || !category || !quantity || isNaN(quantity) || !cost || isNaN(cost)){
...
const HandleUpdateInventory = async (req, res) => {
  ...
  try {
    ...
    const { productName, category, quantity, cost } = req.body;
      ...
      const inventoryItem = await InventoryDb.findOne({ _id: id, createdBy: user._id });
      
      if (!inventoryItem) {
        return res.status(404).json({
          message: "Inventory item not found or you don't have permission to update it"
        });
      }
  ...
  const HandleDeleteInventory = async (req, res) => {
    ...
    try {
      ...
      if (!inventoryItem) {
        return res.status(404).json({
          message: "Inventory item not found or you don't have permission to delete itt"
        });
      }
...
module.exports= {
    HandleAddInventory,HandleDeleteInventory,HandleGetInventory,HandleUpdateInventory
}